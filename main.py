...
def save_to_supabase(
    ...
    lvl_text = f"_Level_({diff})" if diff else ""
    safe_topic = clean_topic.replace(" ", "").replace("/", "_")

    file_base = f"Grade_({yr_short})_Subject_({subject}){lvl_text}_Topic_({safe_topic})_Set_({set_num})"
